...
{
    public class XmlAggregationEventFormatter : IEventFormatter<XElement>
    {
        ...
        public XElement Format(EpcisEvent epcisEvent)
        {
            ...
            AddParentId(epcisEvent, element);
        ...
        {
            if (epcisEvent.Epcs.All(e => e.Type != EpcType.ParentId)) return;

            element.Add(new XElement("parentID", epcisEvent.Epcs.First(x => x.Type == EpcType.ParentId).Id));
        }

        private static void AddEpcList(EpcisEvent epcisEvent, XContainer element)
        {
            var epcList = new XElement("childEPCs");
            var epcQuantity = new XElement("childQuantityList");
            foreach (var epc in epcisEvent.Epcs.Where(x => x.Type == EpcType.ChildEpc)) epcList.Add(new XElement("epc", epc.Id));
        ...
        private static void AddBusinessLocation(EpcisEvent epcisEvent, XContainer element)
        {
            ...
            var location = new XElement("bizLocation", new XElement("id", epcisEvent.BusinessLocation.Id));
            if (!string.IsNullOrEmpty(epcisEvent.BusinessLocation.Type))
                location.SetAttributeValue("type", epcisEvent.BusinessLocation.Type);
